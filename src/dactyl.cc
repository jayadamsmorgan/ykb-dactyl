...
int main() {
    ...
    shapes.push_back(
        Union(ConnectHorizontal(d.key_ctrl, d.key_alt),
              ConnectHorizontal(d.key_backspace, d.key_delete),
              ConnectVertical(d.key_ctrl, d.key_delete),
              Tri(d.key_end.GetBottomLeft(), d.key_delete.GetBottomRight(),
    ...
    TransformList slash_bottom_right =
        d.key_slash.GetBottomRight().TranslateFront(0, -5, -3);

    shapes.push_back(
        TriFan(slash_bottom_right,
               {
    ...
    shapes.push_back(
        TriFan(d.key_tilde.GetBottomRight(),
               {
    ...
    shapes.push_back(
        TriFan(d.key_delete.GetTopLeft(), {
    ...
    // Bottom right corner.
    shapes.push_back(
        TriFan(d.key_shift.GetBottomRight(), {
    ...
    TransformList key_plus_top_right_wall =
        d.key_plus.GetTopRight().TranslateFront(0, 3, -3);
    TransformList key_2_top_left_wall =
        d.key_2.GetTopLeft().TranslateFront(0, 3.75, 0);
    TransformList key_2_top_right_wall =
        d.key_2.GetTopRight().TranslateFront(0, 4, -1);
    TransformList key_3_top_right_wall =
        d.key_3.GetTopRight().TranslateFront(0, 3.5, 0);
    TransformList key_4_top_right_wall =
        d.key_4.GetTopRight().TranslateFront(0, 2.2, 0);
    ...
    shapes.push_back(
        TriFan(key_plus_top_right_wall, {
                                            key_2_top_left_wall,
    ...
    //
    {
        struct WallPoint {
            WallPoint(TransformList transforms, Direction out_direction,
                ...
                : transforms(transforms), out_direction(out_direction),
                  extra_distance(extra_distance), extra_width(extra_width) {}
        ...
        for (WallPoint point : wall_points) {
            ...
            case Direction::UP:
                t.AppendFront(
                    TransformList().Translate(0, distance, 0).RotateX(-20));
                break;
            case Direction::DOWN:
                t.AppendFront(
                ...
                break;
            case Direction::LEFT:
                t.AppendFront(TransformList()
                                  .Translate(-1 * distance, 0, 0)
                                  .RotateY(-20));
                break;
            case Direction::RIGHT:
                t.AppendFront(
                    TransformList().Translate(distance, 0, 0).RotateY(20));
                break;
            }
            ...
            Shape s2 = Hull(Cube(.1).Translate(p2),
                            Cube(.1).Translate(p2 + (width * in_v)));
    ...
    std::vector<Shape> screw_holes;
    {
        ...
        glm::vec3 screw_left_bottom =
            d.key_shift.GetBottomLeft().Apply(kOrigin);
    ...
    for (size_t r = 0; r < d.grid.num_rows(); ++r) {
        for (size_t c = 0; c < d.grid.num_columns(); ++c) {
            ...
            if (top) {
                ...
                if (left && top_left) {
                    shapes.push_back(
                        ConnectDiagonal(*top_left, *top, *key, *left));
                }
            }
        }
    }
