...
// This is the distance between the top of the switch plate and the tip of the
// switch stem.
...
public:
    ...
    // A name for debugging purposes. Can also be used to determine the names
    // for key specific output files.
    std::string name;

    TransformList parent_transforms;
    TransformList local_transforms;

    // These add extra width to the switch and offset the locations of the
    // corners (GetTopLeft etc).
    double extra_width_top = 0;
    double extra_width_bottom = 0;
    double extra_width_left = 0;
    double extra_width_right = 0;

    // This makes the switch taller (moves the corner connectors down and
    // increases wall height) while leaving the top plate of the switch at the
    // same point.
    double extra_z = 0;
    ...
    // can be explicitly passed to support cutting out for long keys like enter
    // on the kinesis.
    Shape GetInverseCap(double custom_vertical_length = -1) const;
    Shape GetCap(bool fill_in_cap_path = false) const;

    // This is the outermost conner of the switch. You can specify an offset to
    // scale the point back by the specified x,y amount towards the center of
    // the switch. If you had a centered 2x2 post and you wanted the corner to
    // line up with outmost point of the switch, you could specify an offset of
    // -1.
    ...
    // The same as GetSwitchTransforms but with a more clear name if you are
    // intending to find the middle point.
...
// when the two connectors being hulled don't have a large projection on one
