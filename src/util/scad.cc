...
void WriteComposite(std::FILE *file,
                    ...
                    const std::vector<Shape> &shapes, int indent_level) {
...
Shape Shape::LiteralComposite(const std::string &name,
                              const std::vector<Shape> &shapes) {
    return Shape([=](std::FILE *file, int indent_level) {
        WriteComposite(
            file, [=](std::FILE *) { fprintf(file, "%s", name.c_str()); },
            shapes, indent_level);
    });
}
...
Shape Shape::LiteralPrimitive(const std::string &primitive) {
    return Primitive(
        [=](std::FILE *file) { fprintf(file, "%s", primitive.c_str()); });
}

Shape Cube(const CubeParams &params) {
    return Shape::Primitive([=](std::FILE *file) {
        fprintf(file, "cube (size = [ %.3f, %.3f, %.3f], center = %s);",
                params.x, params.y, params.z, BoolStr(params.center));
    });
}
...
Shape Cube(double size, bool center) { return Cube(size, size, size, center); }

Shape Square(const SquareParams &params) {
    return Shape::Primitive([=](std::FILE *file) {
        fprintf(file, "square (size = [%.3f, %.3f], center = %s);", params.x,
                params.y, BoolStr(params.center));
    });
}
...
Shape Square(double size, bool center) { return Square(size, size, center); }
...
Shape Cylinder(const CylinderParams &params) {
    return Shape::Primitive([=](std::FILE *file) {
        fprintf(file, "cylinder(h = %.3f, r1 = %.3f, r2 = %.3f, center = %s",
                params.h, params.r1, params.r2, BoolStr(params.center));
        if (params.fn.has_value()) {
            fprintf(file, ", $fn = %.3f", params.fn.value());
        }
        fprintf(file, ");");
...
Shape Polyhedron(const std::vector<Point3d> &points,
                 const std::vector<std::vector<int>> &faces, int convexity) {
    return Shape::Primitive([=](std::FILE *file) {
...
Shape Shape::TranslateX(double x) const { return Translate(x, 0, 0); }

Shape Shape::TranslateY(double y) const { return Translate(0, y, 0); }

Shape Shape::TranslateZ(double z) const { return Translate(0, 0, z); }
...
Shape Shape::Rotate(double degrees, double x, double y, double z) const {
    auto write_name = [=](std::FILE *file) {
        fprintf(file, "rotate (a = %.3f, v = [%.3f, %.3f, %.3f])", degrees, x,
                y, z);
    };
    return Shape::Composite(write_name, {*this});
}

Shape Shape::RotateX(double degrees) const { return Rotate(degrees, 1, 0, 0); }

Shape Shape::RotateY(double degrees) const { return Rotate(degrees, 0, 1, 0); }

Shape Shape::RotateZ(double degrees) const { return Rotate(degrees, 0, 0, 1); }
...
Shape Shape::LinearExtrude(const LinearExtrudeParams &params) const {
    auto write_name = [=](std::FILE *file) {
        fprintf(file,
                ...
                params.height, BoolStr(params.center), params.convexity,
                params.twist, params.slices, params.scale);
    };
    return Shape::Composite(write_name, {*this});
}
...
Shape Shape::Scale(double s) const { return Scale(s, s, s); }
...
Shape Shape::OffsetDelta(double delta, bool chamfer) const {
    auto write_name = [=](std::FILE *file) {
        fprintf(file, "offset (delta = %.3f, chamfer = %s)", delta,
                BoolStr(chamfer));
    };
    return Shape::Composite(write_name, {*this});
}
...
Shape Shape::operator-(const Shape &other) const { return Subtract(other); }
...
Shape Shape::Add(const Shape &other) const { return Union(*this, other); }

Shape Shape::operator+(const Shape &other) const { return Add(other); }
...
Shape Import(const std::string &file_name, int convexity) {
    return Shape::Primitive([=](std::FILE *file) {
        if (convexity > 0) {
            fprintf(file, "import (file = \"%s\", convexity = %d);",
                    file_name.c_str(), convexity);
        } else {
